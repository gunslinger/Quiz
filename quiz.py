...
def check_answer():
    ...
    while answer not in ['a', 'b', 'c', 'd']:
        answer = input("Your answer is (please only input a, b, c, or d): ").lower()
    return answer
...
mark is out of 100 and make sure, you are prepared to go infinity and beyond!"""
    ...
    initial_choice = input("Hello %s, Do you want to start quiz now? (Y/n)" % (name))
    ...
    if check_initial_choice(initial_choice):
        ...
        input("Program will quit... \nPress any key to exit")

if __name__ == '__main__':
    main()
    rechoice = True
    while rechoice:
        a_rechoice = input("Would you like to retry test? (Y/n)")
